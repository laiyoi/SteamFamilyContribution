...
public class FamilyService
{
    ...
    public (string Name, List<FamilyMember> Members, ulong FamilyGroupId)? GetFullFamilyInfoAsync()
    {
        ...
        {
            Logger.Info("[FamilyService] 未登录，无法获取家庭组信息");
            return null;
        }

        if (string.IsNullOrEmpty(_clientManager.AccessToken))
        {
            Logger.Info("[FamilyService] 未获取到 Access Token");
            return null;
        }

        Logger.Info("[FamilyService] 请求完整家庭组信息");
        Logger.Info($"[FamilyService] SteamID: {_clientManager.SteamId.ConvertToUInt64()}");
        ...
        try
        {
            ...
            using (dynamic familyGroupsService = WebAPI.GetInterface("IFamilyGroupsService"))
            {
                ...
                if (familyGroupKv != null && familyGroupId > 0)
                {
                    ...
                    Logger.Info($"[FamilyService] 获取到家庭组：{name}");
                    Logger.Info($"[FamilyService] 成员数量：{members.Count}");
                    Logger.Info($"[FamilyService] 家庭组 ID：{familyGroupId}");
                    
                    // 显示成员详细信息
                    foreach (var member in members)
                    {
                        Logger.Info($"[FamilyService]   成员：{member.SteamId}, 昵称：{member.Nickname ?? "未知"}, 角色：{member.RoleName}");
                    }
        ...
        Logger.Info("[FamilyService] 响应中无家庭组信息");
        return null;
    }
