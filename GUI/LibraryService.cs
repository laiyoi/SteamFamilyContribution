...
public class LibraryService
{
    ...
    public List<SharedApp> GetSharedLibraryAppsAsync(ulong steamId, ulong familyGroupId)
    {
        ...
        Logger.Info($"[LibraryService] SteamID: {steamId}");
        Logger.Info($"[LibraryService] 家庭组 ID: {familyGroupId}");
    ...
    private void SaveResponseToFile(KeyValue kvResponse, List<SharedApp> sharedApps)
    {
        try
        {
            ...
            Logger.Info($"[LibraryService] 响应数据已保存到：{filePath}");
            Logger.Info($"[LibraryService] 保存了 {sharedApps.Count} 个游戏的详细信息");
        }
        catch (Exception ex)
        {
            Logger.Info($"[LibraryService] 保存响应数据失败：{ex.Message}");
        }
    }
}