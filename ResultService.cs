...
public class ResultService
{
    ...
    public static void AnalyzeSavedResult(QueryResult result)
    {
        ...
        Console.WriteLine($"查询时间：{result.Timestamp}");
        Console.WriteLine($"家庭组：{result.FamilyGroupName}");
        Console.WriteLine($"成员数量：{result.FamilyMemberCount}");
        Console.WriteLine($"总贡献金额：{result.TotalContribution:F2} {result.Currency}");
        Console.WriteLine($"共享游戏数量：{result.SharedGames.Count}");
        Console.WriteLine();
        ...
        foreach (var member in result.Members.OrderByDescending(m => m.Contribution))
        {
            ...
            Console.WriteLine($"成员：{member.SteamId} ({member.RoleName})");
            Console.WriteLine($"  贡献金额：{member.Contribution:F2} {result.Currency}");
            Console.WriteLine($"  贡献比率：{ratio:F2}%");
            Console.WriteLine($"  拥有游戏数：{member.OwnedGameCount}");
            Console.WriteLine();
        }
